...
Router salesforceRouter() {
  ...
  Response notFound(String message) => jsonResponse(
        <Map<String, String>>[
  ...
  router.get(
    ...
    final Iterable<MockMember> matches =
        mockMembers.where((MockMember m) => m.memberId == memberId);
    if (matches.isEmpty) {
  ...
  router.get(
    ...
    return jsonResponse(<String, Object?>{
      'vouchers':
          vouchers[matches.first.membershipNumber] ?? <Map<String, Object?>>[],
    });
  });
  ...
  router.post(
    ...
    if (processName == 'AccrueStayPoints') {
      ...
      ledgers[member.memberId]?.insert(
        0,
        <String, Object?>{
      ...
      final Map<String, Object?> payload = <String, Object?>{
        ...
        'outputParameters': <String, Object?>{
          'transactionJournalId': 'a0X${DateTime.now().millisecondsSinceEpoch}',
          'points': points,
  ...
  router.post(
    ...
    final Map<String, Object?> contact =
        (body['associatedContactDetails'] as Map<String, Object?>?) ??
            <String, Object?>{};
    return jsonResponse(<String, Object?>{
      'loyaltyProgramMemberId': '0lM5j${DateTime.now().millisecondsSinceEpoch}',
  ...
  router.get('/services/data/<version>/query',
      (Request request, String version) {
    final String soql = request.url.queryParameters['q'] ?? '';
    final RegExpMatch? match =
        RegExp(r"LoyaltyProgramMemberId\s*=\s*'([^']+)'").firstMatch(soql);
  ...
  router.post('/services/data/<version>/sobjects/Case',
      (Request request, String version) async {
    final Map<String, Object?> body = await readJson(request);
    return jsonResponse(<String, Object?>{
